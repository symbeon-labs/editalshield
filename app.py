...
# Page Config
st.set_page_config(
    page_title="EditalShield", page_icon="🛡️", layout="wide", initial_sidebar_state="expanded"
)
            ...
            if text_input:
                with st.spinner("Scanning for IP risks..."):
                    ...
                    protected, _ = protector.generate_protected_memorial(
                        text_input, protection_level=protection_level
                    )
        ...
        if "analysis" in st.session_state:
            ...
            fig = go.Figure(
                data=go.Scatterpolar(
                    r=values, theta=categories, fill="toself", line_color="#00ff00"
                )
            )
            fig.update_layout(
    ...
    if st.button("SEARCH DATABASE"):
        if query:
            matches = matcher.match_project(
                query, sector=None if sector == "All" else sector.lower()
            )
